...
const basePath = (file) => {
	return path_1.default.join(__dirname, "..", "models", file);
};
	...
	if (transcript === true) {
		channel.messages.fetch({
			...
			.then(function(messages) {
				...
				message.channel.send(`I have sent you a DM, transcript if your DMs are opened. Deleting channel in 20 seconds.`)
...
module.exports.ticketSetup = setup
module.exports.ticketStart = start
module.exports.ticketClose = close

const embedBuilder = async (client, title, author) => {
	return new Discord.MessageEmbed()
		.setColor(client.config.discord.accentColor)
		.setTitle(`${title}`)
		.setFooter((await client.language(`A poll created by ${author.tag}`, message)), author.displayAvatarURL({ dynamic: true }));
};

module.exports.pollEmbed = async (client, msg, title, timeout, options, emojiList = defEmojiList.slice(), forceEndPollEmoji = '\u2705') => {
	if (!msg && !msg.channel) return msg.inlineReply(`${client.emotes.error} - ${await client.language('This channel is inaccessible.', msg)}`);
	if (!title) return msg.inlineReply(`${client.emotes.error} - ${await this.client.language('Poll title is not given.', msg)}`);
	if (!options) return msg.inlineReply(`${client.emotes.error} - ${await client.language('Poll options is not given.', msg)}`);
	if (options.length < 2) return msg.inlineReply(`${client.emotes.error} - ${await client.language('Please provide more than one choice.', msg)}`);
	if (options.length > emojiList.length) return msg.inlineReply(`${client.emotes.error} - ${await client.language(`Please provide ${emojiList.length} or less choices.`, msg)}`);
	...
	reactionCollector.on('end', async () => {
		...
		for (const emoji in emojiInfo) text += `\`${emojiInfo[emoji].option}\` - ${await client.language(`\`${emojiInfo[emoji].votes}\` votes`, message)}\n\n`;
